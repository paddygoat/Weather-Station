...
int glasshouse = 0;
char dataToSendArray[1024][64];   // 256 = 4.2 hours.
int serial_unavailable_count =0;
char data_something[64];
...
void check_serial_exists()
{
  ...
  if (Serial.available()) 
  {
    ...
    if (serial_unavailable_count >0)
    {
      for (int i = 0; i < serial_unavailable_count+1; i++) 
      {
        for (int j = 0; j < 64; j++) 
        {
          data_something[j] = dataToSendArray[i][j];
        }
        String data_to_send_something = data_something;
  ...
  if (!Serial.available() && (serial_unavailable_count<1024))
  {
    ...
    if (currentMillisSerial - prevMillis_Serial >= interval_serial_check)
    {
      ...
      for (int j = 0; j < 64; j++) 
      {
        dataToSendArray[serial_unavailable_count][j] = inChar[j];
        data_something[j] = inChar[j];
      }
      // Serial.println("");
      // Serial.println("dataToSendArray:");
